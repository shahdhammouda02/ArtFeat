...
export default function AuctionBidView({ id }: { id: number }) {
  ...
  const [timeLeft, setTimeLeft] = useState({ d: 0, h: 0, m: 0, s: 0, target: new Date() });
  ...
  if (!found) return <div className="max-w-6xl mx-auto px-4 py-16">Not Found</div>;
  ...
  const handleSubmit: React.FormEventHandler<HTMLFormElement> = (e) => {
    ...
    if (n < minAllowed) {
      alert(`Minimum bid is $${minAllowed.toLocaleString()}`);
      return;
    }
        ...
        <div className="h-full flex flex-col justify-between space-y-6">
          ...
          <div className="flex-grow space-y-6">
            <div className="bg-gray-50 rounded-xl p-6 space-y-4">
              ...
              <div className="flex items-center justify-between">
                <span className="text-gray-600 font-medium">Auction Ends in:</span>
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="text-gray-900 font-medium text-sm">
                  {target.toLocaleString("en-GB", { timeZone: "UTC", hour12: false })} GMT
                </span>
              </div>
            </div>

          {/* Countdown Timer */}
          <div className="bg-white border-2 border-sky-100 rounded-xl p-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-4 text-center">Time Remaining</h3>
            <div className="grid grid-cols-4 gap-4 text-center">
              <div className="bg-sky-50 rounded-lg p-3">
                <div className="text-2xl lg:text-3xl font-bold text-sky-600">{String(d).padStart(2, "0")}</div>
                <div className="text-xs font-medium text-gray-600 tracking-wide">DAYS</div>
              </div>
              <div className="bg-sky-50 rounded-lg p-3">
                <div className="text-2xl lg:text-3xl font-bold text-sky-600">{String(h).padStart(2, "0")}</div>
                <div className="text-xs font-medium text-gray-600 tracking-wide">HOURS</div>
              </div>
              <div className="bg-sky-50 rounded-lg p-3">
                <div className="text-2xl lg:text-3xl font-bold text-sky-600">{String(m).padStart(2, "0")}</div>
                <div className="text-xs font-medium text-gray-600 tracking-wide">MINUTES</div>
              </div>
              <div className="bg-sky-50 rounded-lg p-3">
                <div className="text-2xl lg:text-3xl font-bold text-sky-600">{String(s).padStart(2, "0")}</div>
                <div className="text-xs font-medium text-gray-600 tracking-wide">SECONDS</div>
              </div>
            </div>
          </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <Input
                  ...
                  className="h-12 text-lg border-gray-300 focus:border-sky-500 focus:ring-sky-500"
                />
                <p className="text-sm text-gray-500 mt-2">
                  Minimum bid: <span className="font-semibold text-sky-600">${minAllowed.toLocaleString()}</span>
                </p>
              </div>
              <Button
                type="submit"
                className="w-full h-12 bg-sky-500 hover:bg-sky-600 text-white font-semibold text-lg rounded-lg shadow-md transition-all duration-300 hover:shadow-lg"
              >
                Place Bid
